...
function initilizeWidget() {
  ...
  $(function () {
    ...
    var canvas = $("canvas")[0],
      ...
      color = [
        ...
        ["FFF8DC", "cornsilk", ""],
        ["DC143C", "crimson", "white"],
        ["00FFFF", "cyan", ""],
        ["00008B", "darkblue", "white"],
        ["008B8B", "darkcyan", ""],
        ["B8860B", "darkgoldenrod", ""],
        ["A9A9A9", "darkgray", ""],
        ["006400", "darkgreen", "white"],
        ["BDB76B", "darkkhaki", ""],
        ["8B008B", "darkmagenta", "white"],
        ["556B2F", "darkolivegreen", "white"],
        ["FF8C00", "darkorange", ""],
        ["9932CC", "darkorchid", "white"],
        ["8B0000", "darkred", "white"],
        ["E9967A", "darksalmon", ""],
        ["8FBC8F", "darkseagreen", ""],
        ["483D8B", "darkslateblue", "white"],
        ["2F4F4F", "darkslategray", "white"],
        ["00CED1", "darkturquoise", ""],
        ["9400D3", "darkviolet", "white"],
        ["FF1493", "deeppink", ""],
        ["00BFFF", "deepskyblue", ""],
        ["696969", "dimgray", "white"],
        ["1E90FF", "dodgerblue", ""],
        ["B22222", "firebrick", "white"],
        ["FFFAF0", "floralwhite", ""],
        ["228B22", "forestgreen", ""],
        ["FF00FF", "fuchsia", ""],
        ["DCDCDC", "gainsboro", ""],
        ...
        ["C71585", "mediumvioletred", "white"],
        ["191970", "midnightblue", "white"],
        ["F5FFFA", "mintcream", ""],
        ["FFE4E1", "mistyrose", ""],
        ["FFE4B5", "moccasin", ""],
        ["FFDEAD", "navajowhite", ""],
        ...
        ["FF0000", "red", ""],
        ["BC8F8F", "rosybrown", ""],
        ["4169E1", "royalblue", "white"],
        ["8B4513", "saddlebrown", "white"],
        ["FA8072", "salmon", ""],
        ["F4A460", "sandybrown", ""],
        ["2E8B57", "seagreen", ""],
        ["FFF5EE", "seashell", ""],
    ...
    $.each(orup, function (i, orupVal) {
      $("#fieldsetContainer").append(
        '<fieldset id="' +
          orupVal[0] +
          '" class="columnswrap outer" ><legend> <span role="heading" level="1">' +
          logo +
          " Color Contrast Analyzer </span> </legend></fieldset>"
      );
      $.each(foreback, function (i, forebackVal) {
        $("#" + orupVal[0]).append(
          '<fieldset class="column inner"><legend>' +
            forebackVal +
            "ground Color </legend>" +
            '<span><label for="' +
            orupVal[0] +
            forebackVal +
            'Hex">Hex:</label><input type="text" id="' +
            orupVal[0] +
            forebackVal +
            'Hex" maxlength="7" aria-describedby="errMsg"></span>' +
            '<span><label for="' +
            orupVal[0] +
            forebackVal +
            'Rgb">RGB:</label><input type="text" id="' +
            orupVal[0] +
            forebackVal +
            'Rgb" maxlength="13" aria-describedby="errMsg"></span>' +
            '<span class="inputColor"><label for="' +
            orupVal[0] +
            forebackVal +
            'Choose"> Color: </label><input type="color" id="' +
            orupVal[0] +
            forebackVal +
            'Choose" role="button" aria-label=" Choose ' +
            forebackVal.toLowerCase() +
            'groundColor "> </span> <span style="padding:21px 1.5px 1.5px 1.5px"> <button class="pickerbtn" id="' +
            forebackVal.toLowerCase() +
            'ground-picker" aria-label="' +
            forebackVal.toLowerCase() +
            'ground color picker" title="' +
            forebackVal.toLowerCase() +
            'ground color picker"><img src="images/eyedropper.png" alt="" style="height: 2rem;width:2rem"></button></span>' +
            '<span><label for="' +
            orupVal[0] +
            forebackVal +
            'Name"> Name: </label><select id="' +
            orupVal[0] +
            forebackVal +
            'Name"></select></span>' +
            ...
            orupVal[0] +
            forebackVal +
            'Pick" class="pick" aria-disabled="true" disabled title="Upload an image first.">Pick from image</button>' +
            '<span class="lightness"><label for="' +
            orupVal[0] +
            forebackVal +
            'Lite" aria-live="assertive"> Adjust Lightness: </label><span><input type="range" id="' +
            orupVal[0] +
            forebackVal +
            'Lite" min="0" max="100">' +
      ...
      $("#" + orupVal[0]).append(
        '<fieldset class="column inner"><legend> Result </legend>' +
          ...
          '<tr><th scope="row">AA</th><td id="' +
          orupVal[0] +
          'SmallAA"></td><td id="' +
          orupVal[0] +
          'LargeAA"></td></tr>' +
          '<tr><th scope="row">AAA</th><td id="' +
          orupVal[0] +
          'SmallAAA"></td><td id="' +
          orupVal[0] +
          'LargeAAA"></td></tr>' +
    ...
    $('select[id$="ForeName"]')
      .find('option[value="#FFFFFF"]')
      .attr("selected", true);
    $('select[id$="BackName"]')
      .find('option[value="#0000FF"]')
      .attr("selected", true);
    ...
    function error(errType, field) {
      ...
      setTimeout(function () {
        // Need setTimeout to set focus on element you just left
    ...
    function hexLong(h) {
      ...
      return (
        "#" +
        Array(3).join(h.substr(1, 1)) +
        Array(3).join(h.substr(2, 1)) +
        Array(3).join(h.substr(3, 1))
      );
    }
    ...
    function update($hexField) {
      ...
      var myForeRgb = $("#" + state + "ForeRgb")
          .val()
          .split(","),
        myBackRgb = $("#" + state + "BackRgb")
          .val()
          .split(","),
      ...
      if (!isNaN(ratio)) {
        $(idPrefix + "Ratio]").html(
          (Math.floor(ratio * 100) / 100).toFixed(2) +
            '<span class="toOne"> : 1<span>'
        ); // Truncate the decimal, don't round it
        if (ratio >= 4.5) {
          $(idPrefix + "SmallAA]")
            .html("pass")
            .addClass("pass");
    ...
    $("span.lightness input").each(function () {
      $(this)
        .parent("span")
        .prev("label")
        .text("Adjust Lightness: " + $(this).val() + "%");
    });
    $(".pick, .zoom, #zoomRange")
      .prop("disabled", true)
    ...
    $("select").change(function () {
      if (
        $(this).attr("id") == "orForeName" ||
        $(this).attr("id") == "orBackName"
      ) {
        var $this = $(this);
    ...
    $("body").on("click", "canvas.active", function (e) {
      ...
      var iconOffset = 7;
      var i = ctx.getImageData(
        e.pageX - this.offsetLeft - iconOffset,
        e.pageY - this.offsetTop + iconOffset,
        1,
        1
      ).data;
  ...
  setTimeout(() => {
    document
      .getElementById("foreground-picker")
      .addEventListener("click", () => {
        ...
        eyeDropper
          ...
          .then((result) => {
            resultElement.value = "";
            resultElement.focus();
            resultElement.value = result.sRGBHex.includes("#")
              ? result.sRGBHex
              : rgb2hex(result.sRGBHex);
    ...
    document
      .getElementById("background-picker")
      .addEventListener("click", () => {
    ...
    document.getElementById("copyCCD").addEventListener("click", () => {
      switch (Number(document.getElementById("copyCriteria").value)) {
        ...
        case 11:
          let msg11 =
            "When the visual focus indicator of an element is inside the component it needs to have a contrast ratio of 3:1 at the minimum with its inner background. From the color codes we can see that the visual focus indicator is not meeting the minimum threshold for the contrast with its inner adjacent color making it difficult for the colorblind and low vision users to identify the focussed element.";
          copyColor("Focus indicator", "Inner adjacent", msg11);
          break;
        case 12:
          let msg12 =
            "When the visual focus indicator of an element is outside the component it needs to have a contrast ratio of 3:1 at the minimum with its outer background. From the color codes we can see that the visual focus indicator is not meeting the minimum threshold for the contrast with its outer adjacent color making it difficult for the colorblind and low vision users to identify the focussed element.";
          copyColor("Focus indicator", "Outer adjacent", msg12);
          break;
        case 13:
          let msg13 =
            "When the color contrast ratio of an interactive element's state with its background is less than 3:1, it becomes difficult for a color blind or low vision user to understand different states of the element.";
          copyColor("Selected state", "Adjacent", msg13);
          break;
        case 14:
          let msg14 =
            "Icons convey crucial information about the element's functionality but when they do not meet the contrast ratio of 3:1 with its background then color blind and low vision users will not be able to perceive or understand the icon's functionality.";
          copyColor("Icon", "Adjacent background", msg14);
          break;
        case 15:
          let msg15 =
            "Graphs convey crucial information about the data but when the colors used in the graphs do not meet the contrast ratio of 3:1 with its adjacent colors, then colorblind and low vision users will not be able to perceive or understand the graph's intent.";
          copyColor("Graphical object", "Adjacent", msg15);
          break;
        case 16:
          let msg16 =
            "Link text is visually differentiated from surrounding body text only by using color, this causes people who are color blind or with low vision to miss those links.";
          copyColor("Link text", "Surrounding text", msg16);
          break;
        case 17:
          let msg17 =
            "\nFocus indicator color :" +
            document.getElementById("orForeHex").value +
      ...
      function copyColor(color1, color2, description) {
        let myMessage =
          ...
          color2 +
          " color: " +
          document.getElementById("orBackHex").value +
          "\nContrast Ratio: " +
          document.getElementById("orRatio").innerText +
          " \n\n" +
          description +
          " \n\n";
